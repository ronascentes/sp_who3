...
DBCC TRACESTATUS (1118,3226,2371,4199) WITH NO_INFOMSGS
GO
...
SELECT TOP 10 'Top CPU Query' as [Category],
	...
	qs.total_used_grant_kb AS [memory grants],
	qs.total_worker_time/qs.execution_count AS [avg_worker_time],
    qs.total_logical_reads/execution_count AS [Avg_Logical_Reads],
    qs.total_used_grant_kb/execution_count AS [avg_memory_grants],
...
SELECT TOP 10 'Top Memory Query' as [Category],
	[db_name] = DB_NAME(t.dbid),
    [sql text] = SUBSTRING(t.text,
		(CASE WHEN qs.statement_start_offset = 0 THEN 0 ELSE qs.statement_start_offset/2 END),
		(CASE WHEN qs.statement_end_offset = -1 THEN DATALENGTH(t.text) ELSE qs.statement_end_offset/2 END - (CASE WHEN qs.statement_start_offset = 0 THEN 0 ELSE qs.statement_start_offset/2 END))),
...
CROSS APPLY sys.dm_exec_query_plan(plan_handle) AS qp
WHERE t.dbid > 4
ORDER BY [db_name], qs.total_used_grant_kb DESC -- top memory
GO  
...
SELECT TOP 10 'Top Duration Query' as [Category],
	[db_name] = DB_NAME(t.dbid),
      [sql text] = SUBSTRING(t.text,
		(CASE WHEN qs.statement_start_offset = 0 THEN 0 ELSE qs.statement_start_offset/2 END),
		(CASE WHEN qs.statement_end_offset = -1 THEN DATALENGTH(t.text) ELSE qs.statement_end_offset/2 END - (CASE WHEN qs.statement_start_offset = 0 THEN 0 ELSE qs.statement_start_offset/2 END))),
...
SELECT TOP 10 'Top Recompiles Query' as [Category],
	[db_name] = DB_NAME(t.dbid),
      [sql text] = SUBSTRING(t.text,
		(CASE WHEN qs.statement_start_offset = 0 THEN 0 ELSE qs.statement_start_offset/2 END),
		(CASE WHEN qs.statement_end_offset = -1 THEN DATALENGTH(t.text) ELSE qs.statement_end_offset/2 END - (CASE WHEN qs.statement_start_offset = 0 THEN 0 ELSE qs.statement_start_offset/2 END))),
...
/************** Missing Indexes ************************************************************************************************************************/
SELECT TOP 10 'Missing Index' as [Category],
